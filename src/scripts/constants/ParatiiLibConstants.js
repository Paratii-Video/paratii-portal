/* @flow */

// Default password for encrypting the wallet if the user is not known
export const DEFAULT_PASSWORD = ''
export const WALLET_KEY_ANON = 'keystore-anon'
export const MNEMONIC_KEY_ANON = 'mnemonic-anon'
export const MNEMONIC_KEY_TEMP = 'mnemonic-temp'
export const PASSWORD_TEMP = 'password-temp'
export const WALLET_KEY_SECURE = 'keystore-secure'
export const RESTORE_ACCOUNT = 'restore account flow'
export const NEW_ACCOUNT = 'new account flow'
export const ACTIVATE_SECURE_WALLET = true // a flag for activating the secure wallet
